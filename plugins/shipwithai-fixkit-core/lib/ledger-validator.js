...
function validateLedger(l) {
  ...
  if (l.multi_repo === true && ['design-repo', 'both'].includes(l.fix_source) && l.root_cause_layer !== 'upstream') {
  ...
  // (3) Consumer-edit invariant twin: a design-repo/both bug must not sit at ANY consumer post-fix
  //     state (fixed/candidate/verified/closed) — a clean terminal there is a false close for a bug
  //     that must escalate to the DS repo. Scoped to POST_ROOTCAUSE_STATES to mirror guard (2).
  if (l.multi_repo === true && ['design-repo', 'both'].includes(l.fix_source) && POST_ROOTCAUSE_STATES.includes(state)) {
