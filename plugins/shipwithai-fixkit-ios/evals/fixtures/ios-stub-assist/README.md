# ios-stub-assist — the ASSIST UI exercise (device handoff)

A SwiftUI/UIKit UI bug the agent **cannot observe** (no device/`~~browser` connector wired), so the
...
- `handoff.json` — an emitted `handoff/v0` for the bug: `symptom_layer: UI`, a device `target`
  (iPhone 16 / iOS 18), ordered `steps` (incl. raising Dynamic Type), a UI `LAYER_METHODS` `assertion`
  (`computed-style`) with an `expected` observable, and `verified_by: null`. A valid *request*.
...
and fills `verified_by` — only then may the ledger advance. The real iOS UI bug is the deferred
