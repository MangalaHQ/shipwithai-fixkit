'use strict';
// shipwithai-fixkit-ios — the deterministic blocking gate for the iOS adapter.
// Run from the repo root:  node plugins/shipwithai-fixkit-ios/tests/run-all.js
// Exit 0 = green (gate passes). Any failure exits non-zero (BLOCKING).
//
// Sections:
//   1. stub lifecycles      (ios-stub-logic red->green; ios-stub-assist handoff+candidate, closed refused)
...
const LOGIC_FIX = path.join(ROOT, 'evals', 'fixtures', 'ios-stub-logic');
const ASSIST_FIX = path.join(ROOT, 'evals', 'fixtures', 'ios-stub-assist');
...
assert(runExpectFail(path.join(LOGIC_FIX, 'reproduce.test.js')), 'logic: reproduce.test.js FAILS against buggy.js (wrong tip reproduced)');
...
assert(validatorsExist, 'core ledger + handoff validators resolvable from the ios gate', CORE_LIB);
if (validatorsExist) {
  ...
  const assistVerified = { state: 'verified', symptom_layer: 'UI', root_cause: 'rc', fix: 'f',
    verification: { method: 'computed-style', capability_tier: 'ASSIST', evidence: 'provider observed on iPhone 16', verified_by: 'cowork' } };
  ...
  const baseHandoff = () => ({ version: 'handoff/v0', bug_id: 'BUG-1', symptom_layer: 'UI',
    target: { env: 'device' }, steps: ['observe'], assertion: { method: 'computed-style', expected: 'no truncation' }, verified_by: null });
  ...
  const bInteract = baseHandoff(); bInteract.assertion.method = 'interaction-assertion'; bInteract.assertion.expected = 'tapping the row pushes the detail screen';
...
if (failures.length === 0) {
  console.log(`PASS — ${pass} checks green. iOS-adapter gate satisfied.`);
  process.exit(0);
} else {
