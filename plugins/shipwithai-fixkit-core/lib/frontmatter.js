...
// It is intentionally not a general YAML parser. See lib/ledger.schema.md.
//
// Lives in lib/ (not tests/) because lib/pattern-miner.js reuses it; tests/lib/frontmatter.js
// is a re-export shim so the gate's require and the parser unit tests stay unchanged.
...
function parseFrontmatter(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (rest === '') {
      // Distinguish an object-parent (followed by indented lines) from an empty scalar.
      const nested = {};
      let hasNested = false;
